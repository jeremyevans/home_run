...
standard library ones, showing you how much time is taken by each,
and the number of times home_run is faster or slower. Output is in
CSV, so an entry like this:

  Date._parse,9.14618,0.32827,27.86

means that:

* The standard library Date._parse took 9.14618 seconds
* home_run's Date._parse took 0.32827 seconds
* home_run's Date._parse method is 27.86 times faster

The --bench option does 100000 iterations for every method it
tests, and takes a long time to run.  The reason for this is that
on Windows, the minimum time resolution is about 15 milliseconds,
so you could not get accurate measurements for a lot of methods
if you used a lower number of iterations.
...
* Get it working on rbx-head and jruby-head.
* Add RDoc documentation for all ruby methods.
