...
task :todo do
  ...
  puts "DateTime Class Methods: #{(scm-hrcm).sort.reject{|m| m.to_s =~ /zone_to_diff|_strptime|to_jd|jd_to|day_fraction|valid_|exist|leap|julian|gregorian|today|\A[no]s\?\z/}.join(', ')}"
